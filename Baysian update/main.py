...
# ‼️ NEW: load sigmas_* right after the hazard arrays
(
...
) = load_data(params)          # <— uses your new load_data
...
for i in range(X_test.size(0) * X_test.size(1)):
    exp_idx  = i // X_test.size(1)
    trial_idx = i %  X_test.size(1)

    ev      = X_test[exp_idx, trial_idx, :, 0].numpy().tolist()
    sigma   = sigmas_test[i]          # ‼️ per-trial noise

    L_haz, _, _, _ = BayesianObserver(ev, mu1=-1, mu2=1,
                                      sigma=float(sigma), hs=hs)
    predicted_bin = np.argmax(L_haz[:, -1])
    true_bin      = hazard_bin_indices[i]
    bayesian_correct.append(predicted_bin == true_bin)
